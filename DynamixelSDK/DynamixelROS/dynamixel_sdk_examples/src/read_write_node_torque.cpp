...
#include "dynamixel_sdk_examples/SetCurrent.h"
...
void setCurrentCallback(const dynamixel_sdk_examples::SetCurrent::ConstPtr & msg)
{
  uint8_t dxl_error = 0;
  int dxl_comm_result = COMM_TX_FAIL;

  // Current Value of X series is 2 byte data.
  uint16_t current = (unsigned int)msg->current; // Convert int16 -> uint16

  // Write Goal Current (length : 2 bytes)
  // When writing 2 byte data to AX / MX(1.0), use write2ByteTxRx() instead.
  dxl_comm_result = packetHandler->write2ByteTxRx(
    portHandler, (uint8_t)msg->id, ADDR_GOAL_CURRENT, current, &dxl_error);
  if (dxl_comm_result == COMM_SUCCESS) {
    ROS_INFO("setCurrent : [ID:%d] [CURRENT:%d]", msg->id, msg->current);
  } else {
...
int main(int argc, char ** argv)
{
  ...
  dxl_comm_result = packetHandler->write1ByteTxRx(portHandler, DXL1_ID, ADDR_OPERATING_MODE, CURRENT_CONTROL_MODE, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS)
  {
  ...
  {
    printf("Operating mode changed to current control mode. \n");
  }
  ...
  int32_t pwm = 0;
  dxl_comm_result = packetHandler->read4ByteTxRx(
    portHandler, DXL1_ID, ADDR_GOAL_POSITION, (uint32_t *)&pwm, &dxl_error);
  if (dxl_comm_result == COMM_SUCCESS) {
    ROS_INFO("getPWM : [PWM:%d]", pwm);
  } else {
    ROS_INFO("Failed to get pwm! Result: %d", dxl_comm_result);
  }
